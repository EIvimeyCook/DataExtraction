...
g1<-ggplot(MD_dat, aes(x = fct_rev(data_present), fill = data_present)) +
  geom_bar() +
  theme_pilot() + 
...
g2<-ggplot(MD_dat, aes(x = fct_rev(location_given), fill = location_given)) +
  geom_bar() +
  theme_pilot() + 
  theme(legend.position = "none",
        axis.title.x = element_blank()) +
...
g3<-ggplot(MD_dat, aes(x = fct_rev(figures_present), fill = figures_present)) +
  geom_bar() +
  theme_pilot() + 
  theme(legend.position = "none") +
  labs(x = "Answer", y = "Number") +
...
g4<-ggplot(MD_dat, aes(x = fct_rev(caldat), fill = caldat)) +
  geom_bar() +
  theme_pilot() + 
  theme(legend.position = "none") +
  labs(x = "Answer", y = "Number") +
...
ggsave("MD_bars.jpg", width = 8, height = 8, dpi = 600, device = "jpg")
